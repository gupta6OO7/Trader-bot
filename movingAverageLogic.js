...
// Function to check moving average crossover strategy
const evaluateMovingAverage = (prices) => {
    if (prices.length < 20) {
        console.log("Not enough data for moving average strategy.");
        return null;
    }

    const shortTermMA = movingAverage(prices, 5);  // Short-term MA (5 periods)
    const longTermMA = movingAverage(prices, 20);  // Long-term MA (20 periods)

    console.log(`Short-term MA: ${shortTermMA.toFixed(2)}, Long-term MA: ${longTermMA.toFixed(2)}`);

    if (shortTermMA > longTermMA && config.position === 0) {
        return 'buy';
    } else if (shortTermMA < longTermMA && config.position > 0) {
        return 'sell';
    }
...
const evaluateTrade = async () => {
    ...
    const action = evaluateMovingAverage(priceHistory);
    if (action === 'buy') {
        buyStock(price);
    } else if (action === 'sell') {
        sellStock(price);
